...
public class DummyImpl extends EntityPlayer implements IDummy {
	...
	public DummyImpl(final MinecraftServer server, final WorldServer world, final Location location, final SkinInfo skinInfo) {
		...
		this.d.setGameMode(EnumGamemode.a);
		try {
			this.networkManager = new EmptyNetworkManager(EnumProtocolDirection.b);
			this.b = new EmptyNetworkHandler(server, networkManager, this);
			networkManager.setPacketListener(b);
		} catch (IllegalAccessException e) {
			throw new RuntimeException(e);
		}
		getDataWatcher().set(new DataWatcherObject<>(17, DataWatcherRegistry.a), SKIN_BIT_LAYER);
		new BukkitRunnable() {
			@Override
			public void run() {
	...
	public void display(final Player player) {
		...
		playerConnection.sendPacket(new PacketPlayOutPlayerInfo(EnumPlayerInfoAction.a, this));
		playerConnection.sendPacket(new PacketPlayOutNamedEntitySpawn(this));
		new BukkitRunnable() {
			@Override
			public void run() {
				playerConnection.sendPacket(new PacketPlayOutPlayerInfo(EnumPlayerInfoAction.e, DummyImpl.this));
			}
