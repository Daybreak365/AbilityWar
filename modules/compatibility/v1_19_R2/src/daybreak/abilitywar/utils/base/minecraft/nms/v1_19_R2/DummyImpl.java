package daybreak.abilitywar.utils.base.minecraft.nms.v1_19_R2;
...
import daybreak.abilitywar.utils.base.minecraft.nms.v1_19_R2.network.EmptyNetworkHandler;
import daybreak.abilitywar.utils.base.minecraft.nms.v1_19_R2.network.EmptyNetworkManager;
...
public class DummyImpl extends EntityPlayer implements IDummy {
	...
	public DummyImpl(final MinecraftServer server, final WorldServer world, final Location location, final SkinInfo skinInfo) {
		super(server, world, createProfile(skinInfo));
	...
	public void display(final Player player) {
		...
		playerConnection.send(new ClientboundPlayerInfoUpdatePacket(ClientboundPlayerInfoUpdatePacket.a.ADD_PLAYER, this));
		playerConnection.send(new PacketPlayOutNamedEntitySpawn(this));
		hologram.display(player);
		new BukkitRunnable() {
			@Override
			public void run() {
				playerConnection.send(new ClientboundPlayerInfoRemovePacket(Arrays.asList(DummyImpl.this.getUniqueID())));
			}
