...
class NMS {
	companion object INSTANCE : iNMS {
		private val INSTANCE: iNMS = try {
			Class.forName("daybreak.abilitywar.utils.base.minecraft.compat." + ServerVersion.getVersion().name + ".nms.NMSImpl").asSubclass(iNMS::class.java).getConstructor().newInstance()
		} catch (e: Exception) {
