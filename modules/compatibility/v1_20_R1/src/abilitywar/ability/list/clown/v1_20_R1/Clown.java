package abilitywar.ability.list.clown.v1_20_R1;

import abilitywar.utils.base.minecraft.nms.v1_20_R1.ChannelUtil;
...
public class Clown extends AbstractClown {
	...
	static {
		try {
			BYTE_DATA_WATCHER_OBJECT = FieldUtil.getStaticValue(Entity.class, "DATA_SHARED_FLAGS_ID");
	...
	private void injectPlayer(CraftPlayer player) {
		...
		if (channelHandlers.containsKey(player.getUniqueId())) {
			...
			if (!pair.getLeft().isValid()) {
				try {
					ChannelUtil.getChannel(player.getHandle()).pipeline().remove(pair.getRight());
				} catch (NoSuchElementException ignored) {
				}
			} else return;
		}
