package daybreak.abilitywar.utils.base.minecraft.damage.v1_19_R3;
...
public class DamageImpl implements IDamages {
	...
	public boolean damageExplosion(@NotNull Entity entity, @NotNull Entity source, float damage) {
		...
		final Explosion explosion = new Explosion(
				nmsEntity.getLevel(), nmsSource, nmsSource.getX(), nmsSource.getY(), nmsSource.getZ(), 0f, false, Explosion.Effect.KEEP
		);
	...
	public boolean damageThorn(@NotNull Entity entity, @NotNull LivingEntity damager, float damage) {
		...
		return nmsEntity.hurt(new DamageSource(Holder.direct(new DamageType("thorns", DamageScaling.NEVER, 0)), nmsEntity, ((CraftLivingEntity) damager).getHandle()) {
			{
				setThorns();
				setMagic();
			}
