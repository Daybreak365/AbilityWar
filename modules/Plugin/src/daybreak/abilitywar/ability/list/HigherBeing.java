...
@AbilityManifest(name = "상위존재", rank = Rank.B, species = Species.OTHERS, explain = {
		...
		"자신과 적의 §by값§8(§7높이§8)§f 차이에 따라 공격력이 변화합니다.",
		"나 §e>§f 적 → §c공격력 증가§8(§7차이 반비례§8) §3/§f 나 §e<§f 적 → §d공격 불가 §3/§f 나 §e=§f 적 → §b변동 없음"
})
@Tips(tip = {
	...
	private void onEntityDamageByEntity(EntityDamageByEntityEvent e) {
		...
		if (damager.equals(getPlayer())) {
			...
			if (victimLocationY < damagerLocationY) {
				e.setDamage(Math.floor((e.getDamage() + (e.getDamage() * (1 / (damagerLocationY - victimLocationY + 1) * 0.8))) * 10) / 10);
