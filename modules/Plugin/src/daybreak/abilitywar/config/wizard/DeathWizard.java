...
public class DeathWizard extends SettingWizard {
	...
	static {
		...
		autoRespawnMeta.setDisplayName(ChatColor.translateAlternateColorCodes('&', "&b자동 리스폰"));
		...
		for (int i = 0; i < 27; i++) {
			switch (i) {
				case 11: {
					...
					List<String> lore = Messager.asList(ChatColor.translateAlternateColorCodes('&', "&f게임 진행 중 플레이어가 사망했을 때 어떤 작업을 수행할지 설정합니다."), "");
					OnDeath operation = DeathSettings.getOperation();
					for (OnDeath onDeath : OnDeath.values()) {
						lore.add("");
						lore.add(ChatColor.translateAlternateColorCodes('&', (operation.equals(onDeath) ? "&a" : "&7") + onDeath.name()));
				...
				case 15: {
					...
					meta.setLore(Messager.asList(
							ChatColor.translateAlternateColorCodes('&', "&a활성화&f하면 게임이 시작되고 난 후 사망할 경우 플레이어를 자동으로 리스폰시킵니다."),
							"",
							ChatColor.translateAlternateColorCodes('&', "&7상태 : " + (DeathSettings.getAutoRespawn() ? "&a활성화" : "&c비활성화"))));
