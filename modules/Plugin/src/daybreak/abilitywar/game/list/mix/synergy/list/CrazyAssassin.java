...
@AbilityManifest(name = "정신나갈거같아", rank = Rank.S, species = Species.HUMAN, explain = {
		...
		"상태로 철괴를 우클릭하면 대상의 등 뒤로 순간이동한 후 대상을 빠르게",
		"50번 연타합니다. $[COOLDOWN_CONFIG]"
})
public class CrazyAssassin extends Synergy implements ActiveHandler {

	public static final SettingObject<Integer> COOLDOWN_CONFIG = synergySettings.new SettingObject<Integer>(CrazyAssassin.class, "cooldown", 30,
			"# 쿨타임") {
	...
	public CrazyAssassin(Participant participant) {
		super(participant);
	}
	...
	private final double damage = DAMAGE_CONFIG.getValue();
	...
	private class Attack extends AbilityTimer {

		private final Player target;

		private Attack(final Player target) {
