...
@AbilityManifest(name = "흑기사", rank = Rank.L, species = Species.HUMAN, explain = {
		...
		"§7충전형 무기 §8- §3랜스§f: 말에 탑승한 상태에서 10초마다 랜스가 준비됩니다.",
		...
		" 전환합니다. §e흡수 체력§f은 5초간 지속되며, 이후 급격히 사라집니다."
})
public class BlackKnight extends Synergy implements ActiveHandler {
	...
	private Horse horse;
	private final Style horseStyle = Style.NONE;
	private final Color horseColor = Color.BLACK;

	public BlackKnight(Participant participant) {
		super(participant);
	}
	...
	private enum Tier {
		RELAXED(BarColor.GREEN, new AttributeModifier("add-horse-speed", .4, Operation.ADD_SCALAR), new Note[] {
			...
			@Override
			BlackKnight.Tier previousTier() {
				return null;
			}
			@Override
			BlackKnight.Tier nextTier() {
				return POISED;
			}
	...
	private class HighSpeedDash extends AbilityTimer implements Listener {
		...
		protected void run(int count) {
			...
			if (!dashing) {
				...
				if (--charge > 0) {
					final Horse horse = BlackKnight.this.horse;
	...
	private class Absorption extends AbilityTimer implements Listener {
		...
		private Absorption(final double amount) {
			...
			setPeriod(TimeUnit.TICKS, 1);
			setInitialDelay(TimeUnit.SECONDS, 5);
			start();
		}
	...
	private final Charge horseCharge = new Charge("말", 20, null), lanceCharge = new Charge("랜스", 10, this::isMounted);
	...
	public boolean mount() {
		...
		horse.getAttribute(Attribute.GENERIC_MAX_HEALTH).setBaseValue(120);
		horse.setHealth(120);
		horse.setAdult();
