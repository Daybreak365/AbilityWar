...
public class MixAbilityChanger {
	...
	private static final Logger logger = Logger.getLogger(MixAbilityChanger.class.getName());

	private final Game game;
	private final int period;
	private final GameTimer timer;

	MixAbilityChanger(Game game) {
		this.game = game;
	...
	public void ChangeAbility() {
		for (Participant participant : setupParticipants()) {
			...
			try {
				((Mix) participant.getAbility()).setAbility(abilities.get(random.nextInt(abilities.size())), abilities.get(random.nextInt(abilities.size())));
				Notice(participant);
			} catch (Exception e) {
				logger.log(Level.SEVERE, participant.getPlayer().getName() + "님에게 능력을 할당하는 도중 오류가 발생하였습니다.");
			}
		}
	}
